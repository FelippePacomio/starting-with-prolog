...
/* QUERRIES
 	a. % movie(american_beauty, A).
	b. % movie(M, 2000).
	c. % movie(M, A), A =< 1999.
	d. % movie(M, A), A >= 1990.
	e. % actress(_, scarlett_johansson, _), director(_, N).
	f. % actor(F, N,_), director(F, N).
	g. coestrela(A1,A2,M) :- actor(M,A1,_),actor(M,A2,_), A1 \= A2.
  	   % coestrela(john_goodman,jeff_bridges,M).
   */
...
diretor_ator(FILME,NOME) :- (actor(FILME,NOME,_);actress(FILME,NOME,_)),director(FILME,NOME).
coestrela(A1,A2,M) :- actor(M,A1,_),actor(M,A2,_), A1 \= A2.
lancado_depois(M,ANO) :- movie(M,Y), Y >= ANO.
lancado_antes(M,ANO) :- movie(M,Y), Y =< ANO.
mesmo_ano(M1, M2) :- movie(M1,Y),movie(M2,Y), M1 \= M2.
